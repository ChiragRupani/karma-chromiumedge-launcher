...
export default class BaseBrowser {
  ...
  private excludedFlags: string[];

  constructor(flags: string[], userDataDir: string, excludedFlags: string[]) {
    this.flags = flags || [];
  ...
  _getOptions(url: string): string[] {
    ...
    if (this.excludedFlags?.length > 0) {
      defaultFlags = defaultFlags.filter((flag) => !this.excludedFlags.includes(flag));
    }

    let allflags = [...defaultFlags, ...this.flags, url];
    return allflags;
  }
